...
class MovieRepository {
  // Repository class die doet als een laag tussen de API helper (MovieApi) en de rest van de app. Deze klasse bevat functies die de API helper aanroepen, en kan ook extra logica bevatten zoals data transformatie, caching, of het combineren van meerdere API calls. In dit geval zijn er functies voor het zoeken naar films en het ophalen van volledige details van een film.
  // Zoekfunctionaliteit
  static Future<List<MovieSearchItem>> search(String query) async {
    // Deze functie voert een zoekopdracht uit naar films op basis van een query string. Het roept de search functie van MovieApi aan, en transformeert het resultaat in een lijst van MovieSearchItem objecten, die makkelijker te gebruiken zijn in de rest van de app.
    final data = await MovieApi.search(title: query);

    final results =
        data['results'] as List<dynamic>? ??
        []; // De API response bevat meestal een 'results' veld met een lijst van zoekresultaten. We halen deze lijst op, en zorgen dat het altijd een lijst is (zelfs als er geen resultaten zijn, dan is het een lege lijst).
  ...
  static Future<MovieDetail> getFullMovie(String imdbId) async {
    // Haal zowel show- als episode-granulariteit parallel op, plus OMDb.
    ...
    // Wacht op de Rapid responses en OMDb; geef voorkeur aan episode-data indien beschikbaar.
  ...
  // Rapid detail helpers (verschillende granulariteit)
