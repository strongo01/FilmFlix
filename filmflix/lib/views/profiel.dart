...
class _ProfileScreenState extends State<ProfileScreen> {
              ...
              if (genres is List) {
                for (final g in genres) {
                  if (g is String) {
                    ...
                    if (name is String && name.toLowerCase().contains('advent')) return true;
                  }
                }
              }
            } catch (_) {}
              ...
              if (genres is List) {
                for (final g in genres) {
                  if (g is String) {
                    ...
                    if (lower.contains('horror') || lower.contains('thriller')) return true;
                  } else if (g is Map) {
                    final name = g['name'] ?? g['genre'];
                    if (name is String) {
          ...
          try {
              ...
              for (final v in wm.values) {
                if (v is Map) {
                  ...
                  try {
                    ...
                    DateTime? dt;
                    if (saved is Timestamp) dt = saved.toDate();
                    else if (saved is DateTime) dt = saved;
                    else if (saved is String) dt = DateTime.tryParse(saved);
                    if (dt != null) {
                  ...
                  if (hasMovie) films += 1;
                  final hasAdv = v.any((e) => e is Map && _metaHasAdventure(e));
                  if (hasAdv) adventures += 1;
                  final hasH = v.any((e) => e is Map && _metaHasHorrorThriller(e));
                  if (hasH) horrors += 1;
                  try {
                    for (final e in v) {
                      if (e is Map) {
            ...
            try {
              if (allSaved.isNotEmpty) {
                ...
                while (i < n) {
                  int j = i;
                  while (j + 1 < n && allSaved[j + 1].difference(allSaved[i]).inMinutes <= 10) {
                    j++;
                  }
                  if (j - i + 1 >= 2) {
                    bingeEvents += 1;
                      ...
                      children: [
                        GestureDetector(
                          ...
                          child: CircleAvatar(
                            ...
                            child: CircleAvatar(
                              radius: 50,
                              backgroundColor: _avatarColor ?? Colors.grey.shade200,
                              child: _avatarEmoji != null && _avatarEmoji!.isNotEmpty
                                  ? Text(_avatarEmoji!, style: const TextStyle(fontSize: 32))
                                  : (_displayName != null && _displayName!.isNotEmpty)
                                      ? Text(_displayName!.substring(0, 1).toUpperCase(), style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold))
                                      : Image.network('https://i.pravatar.cc/150?u=kevin', fit: BoxFit.cover),
                            ),
                          ),
                        ),
                        Container(
                    ...
                    SizedBox(
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          _buildBadge(context, 'Avonturier', Icons.explore, Colors.greenAccent, count: _adventureCount, levelBase: 10),
                          _buildBadge(context, 'Horror King', Icons.auto_awesome, Colors.purpleAccent, count: _horrorCount, levelBase: 10),
                          _buildBadge(context, 'Binge Watcher', Icons.bolt, Colors.orangeAccent, count: _bingeCount, levelBase: 10),
                          _buildBadge(context, 'Early Bird', Icons.wb_sunny, Colors.yellowAccent, count: _earlyBirdCount, levelBase: 10),
                        ],
                      ),
                    ),
        ...
        builder: (BuildContext context, StateSetter setModalState) {
          return Padding(
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Pas profielplaatje aan', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 12),
                const Text('Kies kleur', style: TextStyle(fontSize: 13)),
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: selectedColor.value == Colors.grey.shade200.value ? Border.all(color: Colors.black, width: 2) : Border.all(color: Colors.grey.shade300),
                        ),
                    ...
                    ...presetColors.map((c) {
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          width: 44,
                          height: 44,
                          decoration: BoxDecoration(color: c, shape: BoxShape.circle, border: isSelected ? Border.all(color: Colors.white, width: 3) : null),
                        ),
                      );
                    }),
                  ],
                ...
                const SizedBox(height: 12),
                const Text('Kies emoji (optioneel)', style: TextStyle(fontSize: 13)),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 8,
                  children: [
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: selectedEmoji == null ? Colors.black12 : Colors.transparent,
                    ...
                    ...emojis.map((e) {
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          height: 48,
                          alignment: Alignment.center,
                          decoration: BoxDecoration(color: isSelected ? Colors.black12 : Colors.transparent, borderRadius: BorderRadius.circular(8)),
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        ...
                        await docRef.set({
                          'profileAvatar': {'emoji': selectedEmoji ?? '', 'color': colorHex}
  ...
  // Badge Widget
  Widget _buildBadge(BuildContext context, String label, IconData icon, Color color, {double? progress, int? count, int levelBase = 10, bool simpleCount = false}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (count != null) ...[
            if (simpleCount) ...[
              Text(counterText, style: TextStyle(color: primaryText, fontSize: 12, fontWeight: FontWeight.w800)),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text(counterText, style: TextStyle(color: primaryText, fontSize: 11, fontWeight: FontWeight.w700)),
                  if (displayLevel > 1) ...[
                    const SizedBox(width: 6),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                      decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(12)),
                      child: Text('Lv$displayLevel', style: TextStyle(color: color, fontSize: 10, fontWeight: FontWeight.w700)),
                    ),
                  ],
                ],
              ),
