...
class CinemasMapView extends StatefulWidget {
  // Definieert een stateful widget voor de kaartweergave
...
class _CinemasMapViewState extends State<CinemasMapView> {
  // State klasse met mutable data
  bool _loading = true; // Boolean voor laadstatus
  List<Map<String, dynamic>> _cinemas = []; // Lijst met cinema-gegevens
  final MapController _mapController =
      MapController(); // Controller voor kaartbewegingen
  final Color movieBlue = const Color.fromRGBO(
    43,
    77,
    91,
    1,
  ); // Definieert een blauwe kleur constante
  bool _initialized =
      false; // Boolean om eenmalige initialisatie te controleren

  @override
  void initState() {
    // Initialisatiemethode die eenmalig wordt aangeroepen
    super.initState(); // Roept de parent initState aan
  }

  @override
  void didChangeDependencies() {
    // Wordt aangeroepen wanneer dependencies veranderen
    super.didChangeDependencies(); // Roept de parent didChangeDependencies aan
    if (!_initialized) {
      // Controleert of dit al eerder is gedaan
  ...
  Future<void> _loadCinemas() async {
    // Asynchrone methode om cinemas te laden
    final loc = AppLocalizations.of(context)!; // Haalt de taalvertaalhulp op
    try {
      // Start een try-catch block voor foutafhandeling
      final cached = await loadCachedCinemas(); // Laadt cinemas uit de cache
      if (cached.isNotEmpty) {
        // Controleert of er gecachte data is
        setState(() {
          // Gebruikt setState om de UI bij te werken
      ...
      final results =
          await fetchCinemasFromOverpass(); // Haalt verse cinemagevens op van Overpass
      ...
      // Vangt eventuele exceptions
      if (mounted && _cinemas.isEmpty) {
        // Controleert of widget nog bestaat en geen cinemas zijn geladen
        setState(() => _loading = false); // Zet laadstatus op false
      }
      ScaffoldMessenger.of(context).showSnackBar(
        // Toont een snackbar met foutmelding
        SnackBar(
          content: Text(loc.map_load_error(e.toString())),
        ), // Weergeeft de foutmelding
      );
    }
  }

  Future<void> _goToUserLocation() async {
    // Asynchrone methode om naar gebruikerslocatie te gaan
    final contextMounted = mounted; // Slaat op of de widget nog bestaat
    final loc = AppLocalizations.of(context)!; // Haalt de taalvertaalhulp op
    bool serviceEnabled =
        await Geolocator.isLocationServiceEnabled(); // Controleert of locatieservice ingeschakeld is
    if (!serviceEnabled) {
      // Als locatieservice niet ingeschakeld is
      if (contextMounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(loc.map_location_service_disabled)),
        ); // Toont waarschuwing
      return; // Beëindigt de methode
    }

    LocationPermission permission =
        await Geolocator.checkPermission(); // Controleert huidige locatiemachtiging
    if (permission == LocationPermission.denied) {
      // Als machtiging geweigerd is
      permission =
          await Geolocator.requestPermission(); // Vraagt machtiging aan gebruiker
      if (permission == LocationPermission.denied) {
        // Als gebruiker weigert
        if (contextMounted)
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(loc.map_location_permission_denied)),
          ); // Toont waarschuwing
        return; // Beëindigt de methode
      }
    }
    ...
    if (permission == LocationPermission.deniedForever) {
      // Als machtiging permanent geweigerd is
      if (contextMounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(loc.map_location_permission_denied_forever)),
        ); // Toont waarschuwing
      return; // Beëindigt de methode
    }
    ...
    try {
      // Start een try-catch block voor foutafhandeling
      if (contextMounted)
        setState(() => _loading = true); // Zet laadstatus op true
      final pos = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.best,
      ); // Haalt huidige positie op
      _mapController.move(
        LatLng(pos.latitude, pos.longitude),
        15.0,
      ); // Beweegt de kaart naar gebruikerslocatie
    } catch (e) {
      // Vangt eventuele exceptions
      if (contextMounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(loc.map_location_fetch_error(e.toString()))),
        ); // Toont foutmelding
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final markers = _cinemas.map((cinema) {
      // Creëert markers voor iedere cinema
      return Marker(
        // Definieert een kaartmarker
        point: LatLng(
          cinema['lat'] as double,
          cinema['lng'] as double,
        ), // Stelt coördinaten in
        width: 50, // Breedte van marker
        height: 50, // Hoogte van marker
        child: GestureDetector(
          // Maakt marker taps gevoelig
          onTap: () async {
            // Handelaar voor taps op marker
            final website =
                cinema['website'] as String?; // Haalt website van cinema op
            if (website != null && website.isNotEmpty) {
              // Controleert of website bestaat
              var uri = Uri.tryParse(
                website,
              ); // Probeert website als URI te parsen
              if (uri == null || uri.scheme.isEmpty) {
                // Als parsing mislukt of geen protocol
        ...
        await launchUrl(uri, mode: LaunchMode.externalApplication);
            ...
            showDialog(
              // Toont dialoogvenster
              context: context, // Context van de dialoog
              builder: (_) => AlertDialog(
                // Maakt een AlertDialog
                title: Text(
                  cinema['name'] ?? loc.unknown,
                ), // Titel met cinémanaam
                content: Text(loc.map_no_website_content), // Inhoud van dialoog
                actions: [
                  // Actieknoppen
                  TextButton(
                    // Definieert een tekstknop
    ...
    return AppBackground(
      // Wrapper voor app-achtergrond
      child: Scaffold(
        // Hoofdscherm-widget
        backgroundColor: Colors.transparent, // Transparante achtergrond
        appBar: PreferredSize(
          // Custom AppBar met aangegeven grootte
        ...
        body:
            _loading // Controleert laadstatus
            ? const Center(
                child: CircularProgressIndicator(),
              ) // Toont laadwiel als aan het laden
            : FlutterMap(
                // Toont kaart als geladen
                ...
                options: const MapOptions(
                  ...
                  initialCenter: LatLng(
                    52.15,
                    5.3,
                  ), // Beginpositie in het midden van Nederland
                ...
                children: [
                  // Lagen van de kaart
                  TileLayer(
                    // Tegellaag voor kaartachtergrond
                    urlTemplate:
                        'https://tile.openstreetmap.org/{z}/{x}/{y}.png', // OpenStreetMap tegels
                    userAgentPackageName:
                        'nl.bioscopen.app', // User-agent pakket naam
                  ),
