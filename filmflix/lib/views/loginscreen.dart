...
class LoginScreen extends StatefulWidget { // Login scherm widget met state
...
class _LoginScreenState extends State<LoginScreen> { // State voor login scherm
  ...
  @override
  void dispose() { // Opruimen bij verwijdering
  ...
  @override
  void initState() { // Initialisatie
    super.initState(); // Roep parent initState aan
    _authSub = FirebaseAuth.instance.authStateChanges().listen((user) { // Luister naar login veranderingen
      if (user != null && // Gebruiker is ingelogd
          mounted && // Widget bestaat nog
          ModalRoute.of(context)?.isCurrent == true) { // Dit scherm is actief
        if (widget.returnAfterLogin) { // Moet teruggaan
          Navigator.of(context).pop(true); // Ga terug naar vorige scherm
        } else { // Anders ga naar home
          Navigator.of(context).pushReplacement( // Vervang huidiig scherm
            MaterialPageRoute(builder: (_) => const MainNavigation()), // Met home scherm
          );
        }
      }
    });
  ...
  Future<void> _submit() async { // Inlog/registratie formulier indienen
    ...
    try { // Probeer in te loggen
      final auth = FirebaseAuth.instance; // Haal Firebase auth op
      if (_isLogin) { // Inloggen
        await auth.signInWithEmailAndPassword( // Inloggen met email/wachtwoord
          email: _emailCtrl.text.trim(), // Email
          password: _passwordCtrl.text, // Wachtwoord
        );
      } else { // Registreren
        final cred = await auth.createUserWithEmailAndPassword( // Maak nieuw account
          email: _emailCtrl.text.trim(), // Email
          password: _passwordCtrl.text, // Wachtwoord
        );
        final user = cred.user; // Haal nieuwe gebruiker op
        if (user != null) { // Gebruiker bestaat
          await user.updateDisplayName(_nameCtrl.text.trim()); // Zet weergavenaam
          await user.reload(); // Herlaad gebruiker gegevens
          final updatedUser = auth.currentUser; // Haal bijgewerkte gebruiker op
          if (updatedUser != null) { // Gebruiker bestaat
            final usersRef = FirebaseFirestore.instance // Haal Firestore collectie op
                .collection('users')
                .doc(updatedUser.uid);
            await usersRef.set({ // Sla gebruiker gegevens op
      ...
      if (!widget.returnAfterLogin) { // Niet teruggaan
        Navigator.of(context).pushReplacement( // Ga naar home
          MaterialPageRoute(builder: (_) => const MainNavigation()),
        );
      }
    } on FirebaseAuthException catch (e) { // Firebase auth fout
      final message = _localizedFirebaseAuthMessage(e); // Vertaal fout bericht
      Fluttertoast.showToast(msg: message); // Toon bericht
    } catch (e) { // Andere fout
      Fluttertoast.showToast(msg: loc.loginSomethingWentWrong); // Toon generieke fout
    } finally { // Ook als geen fouten
      if (mounted) setState(() => _isLoading = false); // Verberg laad indicator
    }
  }

  Future<UserCredential> _signInWithGoogle() async { // Inloggen met Google
    final loc = AppLocalizations.of(context)!; // Haal vertalingen op
    if (kIsWeb) { // Web platform
      final googleProvider = GoogleAuthProvider(); // Creëer Google provider
      googleProvider.addScope('email'); // Voeg email scope toe
      googleProvider.setCustomParameters({'prompt': 'select_account'}); // Eis account selectie
      try { // Probeer in te loggen
        final userCred = await FirebaseAuth.instance.signInWithPopup( // Toon inlog popup
          googleProvider,
        );
        await _saveUserDoc(userCred.user); // Sla gebruiker op
        return userCred; // Geef credentials terug
      } on FirebaseAuthException catch (e) { // Firebase fout
    ...
    try { // Mobiel platform
      ...
      final googleUser = await googleSignIn.authenticate(); // Vraag authenticatie
      if (googleUser == null) { // Gebruiker geannuleerd
        throw FirebaseAuthException( // Gooi fout
      ...
      try { // Probeer access token te krijgen
        final scopes = <String>['openid', 'email', 'profile']; // Gewenste scopes
        var authorization = await googleUser.authorizationClient // Vraag toestemming
            .authorizationForScopes(scopes);
        authorization ??= await googleUser.authorizationClient.authorizeScopes( // Of vraag opnieuw
          scopes,
        );
        accessToken = authorization.accessToken; // Haal access token op
      } catch (_) { // Fout bij ophalen
        accessToken = null; // Zet null
      }

      final credential = GoogleAuthProvider.credential( // Creëer credentials
        idToken: idToken,
        accessToken: accessToken,
      );

      final userCred = await FirebaseAuth.instance.signInWithCredential( // Inloggen met credentials
        credential,
      );
      await _saveUserDoc(userCred.user); // Sla gebruiker op
      return userCred; // Geef credentials terug
      ...
      rethrow; // Gooi door
    } on PlatformException catch (e) { // Platform fout
      if (e.code.toLowerCase().contains('cancel')) { // Gebruiker geannuleerd
        throw FirebaseAuthException( // Gooi fout
          code: 'sign_in_cancelled',
          message: loc.googleSignInCancelled,
        );
      }
      Fluttertoast.showToast(msg: e.message ?? loc.googleSignInFailed); // Toon fout
      rethrow; // Gooi door
    }
  }

  void _handleFirebaseAuthError(FirebaseAuthException e) { // Behandel Firebase auth fout
    final loc = AppLocalizations.of(context)!; // Haal vertalingen op
    String message = e.message ?? loc.authenticationFailed; // Standaard bericht
    switch (e.code) { // Controleer fout code
  ...
  String _localizedFirebaseAuthMessage(FirebaseAuthException e) { // Vertaal Firebase fout
    final loc = AppLocalizations.of(context)!; // Haal vertalingen op
    String message = e.message ?? loc.authenticationFailed; // Standaard bericht
    switch (e.code) { // Controleer fout code
  ...
  Future<void> _signInWithGitHub() async { // Inloggen met GitHub
    final loc = AppLocalizations.of(context)!; // Haal vertalingen op
    ...
    try { // Probeer in te loggen
      ...
      if (kIsWeb) { // Web platform
        userCred = await FirebaseAuth.instance.signInWithPopup(provider); // Toon popup
      } else { // Mobiel platform
        userCred = await FirebaseAuth.instance.signInWithProvider(provider); // Gebruik provider
      }

      if (userCred.user != null) { // Gebruiker bestaat
        await _saveUserDoc(userCred.user); // Sla gebruiker op
        if (mounted) { // Widget bestaat nog
          if (widget.returnAfterLogin) { // Moet teruggaan
      ...
      debugPrint('GitHub Sign-In Error: ${e.code} - ${e.message}'); // Debug print
      final message = _localizedFirebaseAuthMessage(e); // Vertaal fout
      Fluttertoast.showToast(msg: message); // Toon fout
    } catch (e) { // Andere fout
  ...
  Future<UserCredential> signInWithApple() async { // Inloggen met Apple
    final loc = AppLocalizations.of(context)!; // Haal vertalingen op
    final rawNonce = generateNonce(); // Genereer nonce
    final nonce = sha256ofString(rawNonce); // Hash nonce
    try { // Probeer in te loggen
      final appleCredential = await SignInWithApple.getAppleIDCredential( // Vraag Apple login
        scopes: [ // Gevraagde gegevens
      ...
      if (appleCredential.identityToken == null) { // Geen token
        throw FirebaseAuthException( // Gooi fout
          code: 'null_identity_token',
          message: loc.appleSignInNoIdentityToken,
        );
      }
      final oauthCredential = OAuthProvider("apple.com").credential( // Creëer OAuth credentials
        idToken: appleCredential.identityToken!,
        rawNonce: rawNonce,
        accessToken: appleCredential.authorizationCode,
      );
      final userCredential = await FirebaseAuth.instance.signInWithCredential( // Inloggen met credentials
        oauthCredential,
      );
      if (mounted) { // Widget bestaat nog
        if (widget.returnAfterLogin) { // Moet teruggaan
      ...
      final userBefore = FirebaseAuth.instance.currentUser; // Haal huidge gebruiker op
      try { // Probeer displayName in te stellen
        ...
        if (userBefore != null && // Gebruiker bestaat
            ...
            (userBefore.displayName == null || // Geen displayName
                userBefore.displayName!.trim().isEmpty)) { // Of displayName leeg
          await userBefore.updateDisplayName(given.trim()); // Zet displayName
          await userBefore.reload(); // Herlaad gebruiker
        }
      } catch (e) { // Fout bij updaten
        debugPrint('Apple sign-in: kon displayName niet updaten: $e'); // Debug print
      }

      final userAfter = FirebaseAuth.instance.currentUser; // Haal huidge gebruiker op
      try { // Probeer gebruiker document op te slaan
        final uid = userAfter?.uid; // Haal UID op
        final given = appleCredential.givenName?.trim(); // Haal voornaam op
        if (uid != null && given != null && given.isNotEmpty) { // UID en voornaam bestaan
          final usersRef = FirebaseFirestore.instance // Haal Firestore collectie op
              .collection('users')
              .doc(uid);
          ...
          final shouldSet = // Bepaal of document opgeslagen moet worden
              ...
              (doc.data()?['displayName'] == null || // Geen displayName
                  (doc.data()?['displayName'] as String).trim().isEmpty); // Of displayName leeg
          if (shouldSet) { // Moet opslaan
            await usersRef.set({ // Sla gebruiker document op
              'displayName': given,
  ...
  Future<void> _resetPassword() async { // Wachtwoord reset
    final loc = AppLocalizations.of(context)!; // Haal vertalingen op
    final email = _emailCtrl.text.trim(); // Haal email op
    if (email.isEmpty || !EmailValidator.validate(email)) { // Email niet geldig
      ScaffoldMessenger.of( // Toon bericht
        context,
      ).showSnackBar(SnackBar(content: Text(loc.loginEnterValidEmail)));
      return; // Stop
    }
    try { // Probeer reset email te sturen
      await FirebaseAuth.instance.sendPasswordResetEmail(email: email); // Stuur reset email
      ScaffoldMessenger.of( // Toon succes bericht
        context,
      ).showSnackBar(SnackBar(content: Text(loc.loginPasswordResetEmailSent)));
    } on FirebaseAuthException catch (e) { // Firebase fout
      final message = _localizedFirebaseAuthMessage(e); // Vertaal fout
      ScaffoldMessenger.of(context).showSnackBar( // Toon fout bericht
        SnackBar(content: Text(message)),
      );
    }
  }

  Future<void> _saveUserDoc(User? user, {String? displayName}) async { // Sla gebruiker document op
    if (user == null) return; // Stop als geen gebruiker
    final name = (displayName ?? user.displayName)?.trim(); // Haal naam op
    try { // Probeer op te slaan
      final usersRef = FirebaseFirestore.instance // Haal Firestore collectie op
          .collection('users')
          .doc(user.uid);
      await usersRef.set({ // Sla document op
        if (name != null && name.isNotEmpty) 'displayName': name, // Als naam bestaat
        'email': user.email,
  ...
  Widget _buildCustomTextField({ // Bouw aangepast invoer veld
    ...
    return TextFormField( // Creëer formulier veld
      ...
      decoration: InputDecoration( // Zet stijl
        ...
        fillColor: const Color(0xFF22404B).withOpacity(0.5), // Donker blauw achtergrond
        border: OutlineInputBorder( // Zet border
          borderRadius: BorderRadius.circular(15), // Ronde hoeken
          borderSide: BorderSide.none, // Geen border
        ),
        enabledBorder: OutlineInputBorder( // Border wanneer actief
          borderRadius: BorderRadius.circular(15),
          borderSide: BorderSide(color: Colors.white.withOpacity(0.1)), // Subtiele border
        ),
        focusedBorder: OutlineInputBorder( // Border wanneer gericht
          borderRadius: BorderRadius.circular(15),
          borderSide: const BorderSide(color: Color(0xFFFFC107), width: 1), // Gele border
        ),
      ),
  ...
  @override
  Widget build(BuildContext context) { // Bouw de UI
    const Color scaffoldBgColor = Color(0xFF22404B); // Definieer achtergrondkleur
    ...
    return Scaffold( // Creëer basis scherm
      backgroundColor: scaffoldBgColor, // Zet achtergrondkleur
      body: SafeArea( // Voeg veilige gebied toe
        child: Center( // Centreer inhoud
          child: SingleChildScrollView( // Maak scrollbaar
            padding: const EdgeInsets.all(24.0), // Voeg padding toe
            child: ConstrainedBox( // Beperk maximale breedte
              constraints: const BoxConstraints(maxWidth: 480), // Zet maximale breedte
              child: Card( // Creëer kaart widget
                shape: RoundedRectangleBorder( // Zet afgeronde hoeken
                  borderRadius: BorderRadius.circular(28),
                ),
                elevation: 0, // Verwijder schaduw
                color: cardColor, // Zet kaartkleur
                child: Padding( // Voeg binnenpadding toe
                  padding: const EdgeInsets.all(32.0),
                  child: Form( // Creëer formulier
                    key: _formKey, // Zet formuliersleutel
                    child: Column( // Creëer verticale layout
                      mainAxisSize: MainAxisSize.min, // Minimale hoogte
                      crossAxisAlignment: CrossAxisAlignment.stretch, // Zet volle breedte
                      children: [
                        Center( // Centreer icoon
                          child: Container( // Creëer container
                            padding: const EdgeInsets.all(16), // Voeg padding toe
                            decoration: BoxDecoration( // Zet stijl
                              color: Colors.white.withOpacity(0.1), // Subtiele achtergrond
                              shape: BoxShape.circle, // Maak cirkel
                            ),
                            child: const Icon( // Creëer icoon
                        ...
                        Text( // Creëer titel tekst
                          _isLogin ? loc.loginWelcome : loc.loginCreateAccount, // Zet conditionele titel
                          textAlign: TextAlign.center, // Centreer tekst
                          style: const TextStyle( // Zet tekststijl
                            fontSize: 24,
                        ...
                        if (!_isLogin) ...[ // Toon naam veld alleen bij registratie
                          _buildCustomTextField( // Bouw naam invoerveld
                        ...
                        _buildCustomTextField( // Bouw email invoerveld
                          controller: _emailCtrl,
                          label: loc.loginEmail,
                          icon: Icons.email_outlined,
                          keyboardType: TextInputType.emailAddress,
                          validator: (v) { // Valideer email
                        ...
                        const SizedBox(height: 16), // Voeg verticale spatie toe
                        _buildCustomTextField( // Bouw wachtwoord invoerveld
                          controller: _passwordCtrl,
                          label: loc.loginPassword,
                          icon: Icons.lock_outline,
                          obscureText: _obscure,
                          suffixIcon: IconButton( // Voeg zichtbaarheid knop toe
                            icon: Icon(
                              _obscure
                                  ? Icons.visibility
                            ...
                            onPressed: () =>
                                setState(() => _obscure = !_obscure), // Toggle wachtwoord zichtbaarheid
                          ),
                          validator: (v) { // Valideer wachtwoord
                            if (v == null || v.isEmpty)
                        ...
                        ElevatedButton( // Creëer inlog/registratie knop
                          onPressed: _isLoading ? null : _submit, // Zet onPress handler
                          style: ElevatedButton.styleFrom( // Zet knopstijl
                          ...
                          child: _isLoading // Toon spinner of tekst
                              ? const SizedBox(
                                  height: 20,
                                  width: 20,
                        ...
                        const SizedBox(height: 16), // Voeg verticale spatie toe
                        TextButton( // Creëer toggle login/register knop
                          onPressed: _isLoading
                              ? null
                              : () => setState(() => _isLogin = !_isLogin), // Toggle modus
                          child: Text( // Zet knoptekst
                            _isLogin
                        ...
                        if (_isLogin) // Toon wachtwoord reset knop alleen bij login
                          TextButton( // Creëer wachtwoord reset knop
                            onPressed: _isLoading ? null : _resetPassword, // Zet onPress handler
                            child: Text( // Zet knoptekst
                        ...
                        if (_isLogin) // Toon gast login knop alleen bij login
                          TextButton( // Creëer gast login knop
                            onPressed: _isLoading
                                ? null
                                : () async { // Async handler voor navigatie
                                    setState(() => _isLoading = true); // Toon laad indicator
                                    try { // Probeer navigatie
                                      if (!mounted) return; // Stop als widget weg is
                                      if (widget.returnAfterLogin) { // Moet teruggaan
                                        Navigator.of(context).pop(true); // Pop navigatie
                                      } else { // Anders ga naar home
                                        Navigator.of(context).pushReplacement(
                                          MaterialPageRoute(
                                            builder: (_) =>
                                      ...
                                      if (mounted)
                                        setState(() => _isLoading = false); // Verberg laad indicator
                                    }
                                  },
                            child: Text( // Zet knoptekst
                        ...
                        Row( // Creëer horizontale rij
                          children: [ // Voeg kinderen toe
                            const Expanded( // Voeg expandeerbare divider toe
                              child: Divider(color: Colors.white24),
                            ),
                            Padding( // Voeg voor/na padding toe
                              padding: const EdgeInsets.symmetric(
                                horizontal: 10,
                              ),
                              child: Text( // Creëer scheidingstekst
                                loc.loginOrDivider,
                                style: TextStyle(
                        ...
                        const SizedBox(height: 20), // Voeg verticale spatie toe
                        SignInButton( // Creëer Google inlog knop
                          Buttons.Google,
                        ...
                        if (!kIsWeb && // Toon Apple knop alleen op iOS/macOS
                          ...
                          SignInButton( // Creëer Apple inlog knop
                            Buttons.Apple,
                        ...
                        const SizedBox(height: 12), // Voeg verticale spatie toe
                        SignInButton( // Creëer GitHub inlog knop
                          Buttons.GitHub,
